...
async def add_liquidity_to_pair(router, pair, token0, token1, amount0, amount1, deployer, max_fee):
    ...
    token0_with_account = Contract(token0.address, json.loads(Path("artifacts/abis/ERC20.json").read_text()), deployer)
    print(amount0, type(amount0), amount1, type(amount1))
