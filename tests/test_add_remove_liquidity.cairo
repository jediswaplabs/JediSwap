...
func test_add_liquidity_expired_deadline{syscall_ptr : felt*, range_check_ptr}():
    ...
    IRouter.add_liquidity(contract_address = router_address, tokenA = token_0_address, tokenB = token_1_address, amountADesired = Uint256(amount_token_0, 0), amountBDesired = Uint256(amount_token_1, 0), amountAMin = Uint256(1,0), amountBMin = Uint256(1,0), to = 100, deadline = 0)
    %{ stop_warp() %}
...
@external
func test_add_remove_liquidity_created_pair{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}():
    alloc_locals
    ...
    let (reserve_0 : Uint256, reserve_1 : Uint256, block_timestamp_last) = IPair.get_reserves(contract_address = pair_address)
    ...
    let (is_total_supply_mul_lesser_than_equal_reserve_mul) = uint256_le(totalSupply_mul_totalSupply, reserve_0_mul_reserve_1)
    ...
    let (totalSupply1_mul_totalSupply1 : Uint256) = uint256_checked_mul(totalSupply_plus_1, totalSupply_plus_1)

    let (is_total_supply_1_mul_greater_than_reserve_mul) = uint256_lt(reserve_0_mul_reserve_1, totalSupply1_mul_totalSupply1)
    ...
    %{ stop_prank = start_prank(ids.user_1_address, target_contract_address=ids.router_address) %}
    let (amountA_again : Uint256, amountB_again : Uint256, liquidity_again : Uint256) = IRouter.add_liquidity(contract_address = router_address, tokenA = token_0_address, tokenB = token_1_address, amountADesired = Uint256(amount_token_0, 0), amountBDesired = Uint256(amount_token_1, 0), amountAMin = Uint256(1,0), amountBMin = Uint256(1,0), to = user_1_address, deadline = 0)
    %{ stop_prank() %}
    ...
    let (reserve_0_again : Uint256, reserve_1_again : Uint256, block_timestamp_last_again) = IPair.get_reserves(contract_address = pair_address)
    let (totalSupply_again : Uint256) = IERC20.totalSupply(contract_address = pair_address)

    let (totalSupply_mul_totalSupply_again : Uint256) = uint256_checked_mul(totalSupply_again, totalSupply_again)
    let (reserve_0_mul_reserve_1_again : Uint256) = uint256_checked_mul(reserve_0_again, reserve_1_again)

    let (is_total_supply_mul_lesser_than_equal_reserve_mul_again) = uint256_le(totalSupply_mul_totalSupply, reserve_0_mul_reserve_1)
    assert is_total_supply_mul_lesser_than_equal_reserve_mul_again = 1

    let (user_1_token_0_balance : Uint256) = IERC20.balanceOf(contract_address = token_0_address, account = user_1_address)

    let (expected_reserve_0 : Uint256) = uint256_checked_sub_le(Uint256(amount_to_mint_token_0, 0), user_1_token_0_balance)
    assert expected_reserve_0 = reserve_0_again

    let (user_1_token_1_balance : Uint256) = IERC20.balanceOf(contract_address = token_1_address, account = user_1_address)
    
    let (expected_reserve_1 : Uint256) = uint256_checked_sub_le(Uint256(amount_to_mint_token_1, 0), user_1_token_1_balance)
    assert expected_reserve_1 = reserve_1_again

    let (user_1_pair_balance : Uint256) = IERC20.balanceOf(contract_address = pair_address, account = user_1_address)
    
    let (expected_total_supply : Uint256) = uint256_checked_add(Uint256(MINIMUM_LIQUIDITY, 0), user_1_pair_balance)
    ...
    IERC20.approve(contract_address = pair_address, spender = router_address, amount = user_1_pair_balance)
    %{ stop_prank() %}

    %{ stop_prank = start_prank(ids.user_1_address, target_contract_address=ids.router_address) %}
    let (amountA_burn : Uint256, amountB_burn : Uint256) = IRouter.remove_liquidity(contract_address = router_address, tokenA = token_0_address, tokenB = token_1_address, liquidity = user_1_pair_balance, amountAMin = Uint256(1, 0), amountBMin = Uint256(1, 0), to = user_1_address, deadline = 0)
    %{ stop_prank() %}
    ...
    let (user_1_pair_balance_burn : Uint256) = IERC20.balanceOf(contract_address = pair_address, account = user_1_address)
    ...
    let (burn_address_balance : Uint256) = IERC20.balanceOf(contract_address = pair_address, account = BURN_ADDRESS)
    assert totalSupply_burn = burn_address_balance

    let (reserve_0_burn : Uint256, reserve_1_burn : Uint256, block_timestamp_last_burn) = IPair.get_reserves(contract_address = pair_address)

    let (totalSupply_mul_totalSupply_burn : Uint256) = uint256_checked_mul(totalSupply_burn, totalSupply_burn)
    let (reserve_0_mul_reserve_1_burn : Uint256) = uint256_checked_mul(reserve_0_burn, reserve_1_burn)

    let (is_total_supply_mul_lesser_than_equal_reserve_mul_burn) = uint256_le(totalSupply_mul_totalSupply_burn, reserve_0_mul_reserve_1_burn)
...
@external
func test_add_remove_liquidity_for_non_created_pair{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}():
    alloc_locals
    ...
    let (token_0_address, token_1_address) = IRouter.sort_tokens(contract_address = router_address, tokenA = unsorted_token_0_address, tokenB = unsorted_token_1_address)
    ...
    let (pair_address) = IFactory.get_pair(contract_address=factory_address, token0 = token_1_address, token1 = token_0_address)
