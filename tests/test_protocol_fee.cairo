...
func test_protocol_fee{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}():
    ...
    let (fee_recipient_pair_balance : Uint256) = IERC20.balanceOf(contract_address = pair_address, account = fee_recipient_address)

    let (is_fee_recipient_pair_balance_greater_than_0) = uint256_lt(Uint256(0, 0), fee_recipient_pair_balance)
