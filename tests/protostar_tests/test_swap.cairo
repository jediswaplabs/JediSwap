...
func test_swap_exact_0_to_1{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}():
    ...
    let (amounts_len : felt, amounts : Uint256*) = IRouter.swap_exact_tokens_for_tokens(contract_address = router_address, amountIn = Uint256(amount_token_0, 0), amountOutMin = Uint256(0, 0), path_len = 2,
    path = path, to = user_2_address, deadline = 0)
    %{ stop_prank() %}
    ...
    let (user_2_token_0_balance_final : Uint256) = IERC20.balanceOf(contract_address = token_0_address, account = user_2_address)
    let (user_2_token_1_balance_final : Uint256) = IERC20.balanceOf(contract_address = token_1_address, account = user_2_address)
    let (reserve_0_final : Uint256, reserve_1_final : Uint256, _) = IPair.get_reserves(contract_address = pair_address)

    let (user_2_token_0_balance_difference : Uint256) = uint256_checked_sub_le(user_2_token_0_balance_initial, user_2_token_0_balance_final)
    let (user_2_token_1_balance_difference : Uint256) = uint256_checked_sub_le(user_2_token_1_balance_final, user_2_token_1_balance_initial)
...
func test_swap_0_to_exact_1{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}():
    ...
    %{ stop_prank = start_prank(ids.user_2_address, target_contract_address=ids.token_0_address) %}
    IERC20.approve(contract_address = token_0_address, spender = router_address, amount = Uint256(10 ** 30, 0))
    %{ stop_prank() %}
    ...
    assert [path] = token_0_address
    assert [path + 1] = token_1_address
    let (amounts_len : felt, amounts : Uint256*) = IRouter.swap_tokens_for_exact_tokens(contract_address = router_address, amountOut = Uint256(amount_token_1, 0), amountInMax = Uint256(10 ** 30, 0), path_len = 2,
    path = path, to = user_2_address, deadline = 0)
    %{ stop_prank() %}
...
func test_swap_exact_0_to_2{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}():
    ...
    %{  
        ids.token_0_address = context.sorted_token_0_address
        ids.token_1_address = context.sorted_token_1_address
        ids.token_2_address = deploy_contract("contracts/test/token/ERC20.cairo", [33, 3, 18, 3000]).contract_address
        ids.factory_address = context.factory_address
        ids.router_address = context.router_address
    ...
    IERC20.mint(contract_address = token_2_address, recipient = user_1_address, amount = Uint256(amount_to_mint_token_2, 0))
    ...
    %{ stop_prank = start_prank(ids.user_1_address, target_contract_address=ids.router_address) %}
    let (amountA : Uint256, amountB : Uint256, liquidity : Uint256) = IRouter.add_liquidity(contract_address = router_address, tokenA = token_1_address, tokenB = token_2_address, amountADesired = Uint256(amount_token_1, 0), amountBDesired = Uint256(amount_token_2, 0), amountAMin = Uint256(1,0), amountBMin = Uint256(1,0), to = user_1_address, deadline = 0)
    %{ stop_prank() %}

    let (user_2_token_0_balance_initial : Uint256) = IERC20.balanceOf(contract_address = token_0_address, account = user_2_address)
    let (user_2_token_2_balance_initial : Uint256) = IERC20.balanceOf(contract_address = token_2_address, account = user_2_address)
    let (reserve_0_0_initial : Uint256, reserve_1_0_initial : Uint256, _) = IPair.get_reserves(contract_address = pair_address)
    let (reserve_other_0_initial : Uint256, reserve_other_1_initial : Uint256, _) = IPair.get_reserves(contract_address = other_pair_address)
    ...
    let (amounts_len : felt, amounts : Uint256*) = IRouter.swap_exact_tokens_for_tokens(contract_address = router_address, amountIn = Uint256(amount_token_0, 0), amountOutMin = Uint256(0, 0), path_len = 3,
    path = path, to = user_2_address, deadline = 0)
    %{ stop_prank() %}
    ...
    let (user_2_token_0_balance_final : Uint256) = IERC20.balanceOf(contract_address = token_0_address, account = user_2_address)
    let (user_2_token_2_balance_final : Uint256) = IERC20.balanceOf(contract_address = token_2_address, account = user_2_address)
    let (reserve_0_0_final : Uint256, reserve_1_0_final : Uint256, _) = IPair.get_reserves(contract_address = pair_address)
    let (reserve_other_0_final : Uint256, reserve_other_1_final : Uint256, _) = IPair.get_reserves(contract_address = other_pair_address)
    ...
    let (user_2_token_0_balance_difference : Uint256) = uint256_checked_sub_le(user_2_token_0_balance_initial, user_2_token_0_balance_final)
    let (user_2_token_2_balance_difference : Uint256) = uint256_checked_sub_le(user_2_token_2_balance_final, user_2_token_2_balance_initial)
...
func test_swap_exact_1_to_0{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}():
    ...
    let (amounts_len : felt, amounts : Uint256*) = IRouter.swap_exact_tokens_for_tokens(contract_address = router_address, amountIn = Uint256(amount_token_1, 0), amountOutMin = Uint256(0, 0), path_len = 2,
    path = path, to = user_2_address, deadline = 0)
    %{ stop_prank() %}
    ...
    let (user_2_token_0_balance_difference : Uint256) = uint256_checked_sub_le(user_2_token_0_balance_final, user_2_token_0_balance_initial)
    let (user_2_token_1_balance_difference : Uint256) = uint256_checked_sub_le(user_2_token_1_balance_initial, user_2_token_1_balance_final)
