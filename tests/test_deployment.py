...
@pytest.mark.asyncio
async def test_pair_in_registry(registry, token_0, token_1, pair, other_pair, stable_pair, other_stable_pair):
    ...
    execution_info = await registry.get_all_pairs().call()
    assert execution_info.result.all_pairs == [pair.contract_address, other_pair.contract_address, stable_pair.contract_address, other_stable_pair.contract_address]
